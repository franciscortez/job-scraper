...
export function verifyJobs(jobs, io, started = io.now(), cursors = {}) {
  ...
  for (const job of batch) {
    if (io.now() - started >= RUN_BUDGET_MS) {
      errors.push('Four-minute budget reached; unchecked jobs hidden until verified.');
  ...
  return { results, errors, cursors: nextCursors, limited: ordered.length > batch.length };
}
